...
file_path = ('S:/ES/AACICore/StudentTracker/2024-2025/Spring 2025/Perris Union/10023223_10023223-195254-DETAIL-EFFDT-20250416-RUNDT-20250502.csv')
nsc = pd.read_csv(file_path, low_memory=False)

nsc['Your_Unique_Identifier'] = nsc['Your_Unique_Identifier'].str.strip('_')
nsc['SSID'] = nsc['Your_Unique_Identifier'].astype('int64')
...
nsc2 = pd.read_csv(file_path, low_memory=False)

nsc2 = nsc2[["SSID", "Record_Found_Y/N", "College_Name", "College_State", "2-Year/4-Year", "Public/Private",
             "Enrollment_Begin", 'Enrollment_End', "Enrollment_Status"]]

nsc2_cols = {"Record_Found_Y/N": "Record_Found_Y/N1", "College_Name": "College_Name1",
             "College_State": "College_State1", "2-Year/4-Year": "2-Year/4-Year1",
             "Public/Private": "Public/Private1", "Enrollment_Begin": "Enrollment_Begin1",
             'Enrollment_End': 'Enrollment_End1', "Enrollment_Status": "Enrollment_Status1"}
nsc2.rename(columns=nsc2_cols, inplace=True)

nsc = pd.merge(nsc, nsc2, on='SSID', how='left')

for n_col, n_rep in nsc2_cols.items():
    nsc.loc[nsc[n_col].isnull(), n_col] = nsc[n_rep]
    nsc = nsc.drop([n_rep], axis=1)
...
missing_enrol = pd.read_csv(file_path, low_memory=False)
...
outcomes.to_parquet('PUHSD_post_secondary_by_feeder.parquet', index=False)
outcomes.to_csv('PUHSD_post_secondary_by_feeder.csv', index=False)
